...
void ReadRoot()
{
    ...
    {
    //G4cout << fTree1->GetEntries() << G4endl;
        if (verbose >= 0 && i % 1 == 0)
        {
            clock_t stop = clock();
...
G4cout << "Here!!!!!" << G4endl;
        for (int _i = 0; _i < NX; _i++)
            ...
            {
                if (fSmearXY->GetBinContent(_i + 1, _j + 1) == 0) continue;
                if (_i + 18 >= NX) continue;
                if (_i + 40 >= NX) continue;
                if (_j + 40 >= NY) continue;
                if (_j + 18 >= NY) continue;
                if (i % 2 == 1) data[_i + 18][_j + 40] = (unsigned short)(fSmearXY->GetBinContent(_i + 1, _j + 1));
                else data[_i + 40][_j + 18] = (unsigned short)(fSmearXY->GetBinContent(_i + 1, _j + 1));
		G4cout << data[_i + 18][_j + 40] << G4endl;
            }
