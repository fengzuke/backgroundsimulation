...
const int Nx = 72;
const int Ny = 72;
const double spaRes = 0.04;   //default: 0.07 min:0.02
const double lenPit = 0.08333;
...
void ReadRoot()
{
    ...
    c2 = new TCanvas("c2", "c2", 720, 720);
    c3 = new TCanvas("c3", "c3", 720, 720);
    ...
    fhitXY = new TH2D("fhitXY", "fhitXY", NX, -lenPit * (NX/2), lenPit * (NX/2), NY, -lenPit * (NY/2), lenPit * (NY/2));
    fhitXY2 = new TH2D("fhitXY2", "fhitXY2", Nx, -lenPit * (Nx/2), lenPit * (Nx/2), Ny, -lenPit * (Ny/2), lenPit * (Ny/2));
    fhitXZ = new TH2D("fhitXZ", "fhitXZ", NX, -lenPit * (NX/2), lenPit * (NX/2), NY, -lenPit * NY, 0);
    fhitYZ = new TH2D("fhitYZ", "fhitYZ", NX, -lenPit * (NX/2), lenPit * (NX/2), NY, -lenPit * (NY/2), lenPit * (NY/2));
    ...
    fSmearXY = new TH2D("fSmearXY", "fSmearXY", NX, -lenPit * (NX/2), lenPit * (NX/2), NY, -lenPit * (NY/2), lenPit * (NY/2));
    fSmearXY2 = new TH2D("fSmearXY2", "fSmearXY2", Nx, -lenPit * (Nx/2), lenPit * (Nx/2), Ny, -lenPit * (Ny/2), lenPit * (Ny/2));
    fSmearXZ = new TH2D("fSmearXZ", "fSmearXZ", NX, -lenPit * (NX/2), lenPit * (NX/2), NY, -lenPit * NY, 0);
    fSmearYZ = new TH2D("fSmearYZ", "fSmearYZ", NX, -lenPit * (NX/2), lenPit * (NX/2), NY, -lenPit * (NY/2), lenPit * (NY/2));
    ...
    for (int i = 0; i < fTree1->GetEntries(); i++)
    {
        ...
        double PEPositionX = fEvent->GetPEPosition().x();
        double PEPositionY = fEvent->GetPEPosition().y();
        //double PEPositionZ = fEvent->GetPEPosition().z();
        double ay1 = 0, ay2 = 0, ay3 = 0, ay4 = 0, ay5 = 0, ay6 = 0;
        double k1, k2, k3;
        ofstream mctheta;
        mctheta.open("mctheta.txt",ios::app);
        if (PEDDirectionX == 0)
        {
            if (PEDDirectionY > 0) ay2 = TMath::Pi()/2;
            else if (PEDDirectionY < 0) ay2 = -TMath::Pi()/2;
            else if (PEDDirectionY == 0) ay2 = 9999;
        }
        else k1 = PEDDirectionY/PEDDirectionX; 
        ...
        if(ay2 == 0) cout<<PEDDirectionX<<" "<<PEDDirectionY<<" "<<PEDDirectionZ<<" "<<k1<<endl;
        if(ay2 == 9999) continue;
        mctheta << i+1 << " " << ay2 << " " << PEPositionX << " " << PEPositionY << endl;
        mctheta.close();

        if (PEDDirectionY == 0)
        {
            if (PEDDirectionZ > 0) ay4 = TMath::Pi()/2;
            else if (PEDDirectionZ < 0) ay4 = -TMath::Pi()/2;
            else if (PEDDirectionZ == 0) ay4 = 9999;
        }
        else k2 = PEDDirectionZ/PEDDirectionY; 
        ...
        if (PEDDirectionX == 0)
        {
            if (PEDDirectionZ > 0) ay6 = TMath::Pi()/2;
            else if (PEDDirectionZ < 0) ay6 = -TMath::Pi()/2;
            else if (PEDDirectionZ == 0) ay6 = 9999;
        }
        else k3 = PEDDirectionZ/PEDDirectionX; 
        ...
        while ((tempObj = it->Next()))
        {
            ...
            int fzk;
            if (verbose > 1  && trk->GetTrackID()==1 && abs(trk->GetExitX())<3  && abs(trk->GetExitY())<3)
            {
                G4cout << ": \t-------------------------" << G4endl;
            ...
            for (int j = 0; j < (int)stepIdx.size(); j++)
            {
                ...
                int ffzk;
                ffzk=dep->GetPDGID();
                if (verbose > 1 && trk->GetTrackID()==1 && abs(dep->GetPostX())<3  && abs(dep->GetPostY())<3 )
                {
                    G4cout << ": \t----hit  : " << j << G4endl;
                    ...
                    d=(((dep->GetPostZ())+5)/10)*2;
                    //G4cout << "d " << d << G4endl;
                }
                if(abs(dep->GetPostX())<3  && abs(dep->GetPostY())<3)
                {
                fhitXY->Fill(dep->GetPostX(), dep->GetPostY(), dep->GetEdep() * conFac);
                //G4cout <<dep->GetPostX()<< dep->GetPostY()<< dep->GetEdep() * conFac<< G4endl;
                fhitXY2->Fill(dep->GetPostX(), dep->GetPostY(), dep->GetEdep() * conFac);
                }


                //cout<<"==>>X:"<<dep->GetPostX()<<"==Y:"<<dep->GetPostY()<<"==Z:"<<dep->GetPostZ()<<"==T:"<<dep->GetPostT()<<"==E:"<<dep->GetEdep()<<endl;
        ...
        double spaRe=spaRes*sqrt(d);
        SmearingGaus(fhitXY, fSmearXY, spaRe);
        SmearingGaus(fhitXY2, fSmearXY2, spaRe);

        for (int _i = 0; _i < NX; _i++)
        {
            for (int _j = 0; _j < NY; _j++)
            {
                data[_i][_j] = (unsigned short)(fSmearXY2->GetBinContent(_i + 1, _j + 1));
            }
        }

        for (int ___i = 0; ___i < Ny; ___i++)
        {
            for (int ___j = 0; ___j < Ny; ___j++)
            {
                if ((fSmearXY2->GetBinContent(___i + 1, ___j + 1))!=0)
                {
        ...
        c2->cd();
        fhitXY->Draw("colz");
        //c2->SaveAs(Form("Root_hitXY_%dEvnent.png",i));
        c3->cd();
        fSmearXY->Draw("colz");
        //c3->SaveAs(Form("Root_hSmearXY_%dEvnent.png",i));
    ...
    //c2->SaveAs(sss1);
    //G4cout << "In totle " << nub<<" has been stored"<< G4endl;
    G4cout << "In totle " << nub2<<" has been stored"<< G4endl;
}
