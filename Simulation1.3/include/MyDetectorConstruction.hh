...
#define ANTICSHIELD3 12
#define ANTICSHIELD4 13
#define ANTICSHIELD5 14
...
public:
  ...
  void SetWindowSizeHalfR(G4double R) { fDetPar[WINDOW]->Siz[0] = R; }
  void SetWindowSizeHalfH(G4double H) { fDetPar[WINDOW]->Siz[1] = H; }
  //void SetWindowSizeHalfZ(G4double z) { fDetPar[WINDOW]->Siz[2] = z; }
  G4double GetWindowSizeHalfR() { return fDetPar[WINDOW]->Siz[0]; }
  G4double GetWindowSizeHalfH() { return fDetPar[WINDOW]->Siz[1]; }
  //G4double GetWindowSizeHalfZ() { return fDetPar[WINDOW]->Siz[2]; }

  void SetGasSizeHalfR(G4double R) { fDetPar[GAS]->Siz[0] = R; }
  void SetGasSizeHalfH(G4double H) { fDetPar[GAS]->Siz[1] = H; }
  //void SetGasSizeHalfZ(G4double z) { fDetPar[GAS]->Siz[2] = z; }
  G4double GetGasSizeHalfR() { return fDetPar[GAS]->Siz[0]; }
  G4double GetGasSizeHalfH() { return fDetPar[GAS]->Siz[1]; }
  //G4double GetGasSizeHalfZ() { return fDetPar[GAS]->Siz[2]; }

  G4bool InGasDetector(const G4ThreeVector aPosition) const { 
    if(pow(aPosition.x(),2)+pow(aPosition.y(),2) <= pow(fDetPar[GAS]->Siz[0],2)*3.1415926 && aPosition.z() <= fDetPar[GAS]->Siz[1]+fDetPar[GAS]->Pos[2] && aPosition.z() >= -fDetPar[GAS]->Siz[1]+fDetPar[GAS]->Pos[2])
      return true; 
